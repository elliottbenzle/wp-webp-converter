...
class OC_WebP_Media_Converter {
	...
	public function render_admin_page() {
		...
		if ( isset( $_POST['ocwc_run_batch'] ) ) {
			...
			$batch_filter = isset( $_POST['batch_filter'] ) ? sanitize_text_field( wp_unslash( $_POST['batch_filter'] ) ) : $defaults['batch_filter'];
			if ( 'all' !== $batch_filter && ! preg_match( '/^\d{4}-\d{2}$/', $batch_filter ) ) {
				$batch_filter = $defaults['batch_filter'];
			}
		...
		$selected_batch_filter = isset( $options['batch_filter'] ) ? (string) $options['batch_filter'] : 'all';
		if ( 'all' !== $selected_batch_filter && ! preg_match( '/^\d{4}-\d{2}$/', $selected_batch_filter ) ) {
				...
				<form method="post">
					...
					<table class="form-table" role="presentation">
						<tr>
							...
							<td>
								<select name="batch_filter" id="batch_filter">
									<option value="all" <?php selected( $selected_batch_filter, 'all' ); ?>>All</option>
									<?php foreach ( $batch_filter_options as $filter_value => $filter_label ) : ?>
										<option value="<?php echo esc_attr( $filter_value ); ?>" <?php selected( $selected_batch_filter, $filter_value ); ?>><?php echo esc_html( $filter_label ); ?></option>
									<?php endforeach; ?>
								</select>
								<p class="description">Choose All to process every remaining JPG/PNG attachment, or choose a month to process only images uploaded during that month.</p>
							</td>
						</tr>
						<tr>
	...
	private function get_batch_filter_options() {
		global $wpdb;

		$sql = "SELECT DISTINCT YEAR(post_date) AS upload_year, MONTH(post_date) AS upload_month
			FROM {$wpdb->posts}
		...
		foreach ( $rows as $row ) {
			$year  = isset( $row->upload_year ) ? (int) $row->upload_year : 0;
			$month = isset( $row->upload_month ) ? (int) $row->upload_month : 0;
			...
			$options[ $value ] = $this->get_batch_filter_label( $value );
		}

		return $options;
	}

	private function get_batch_filter_label( $batch_filter ) {
		if ( 'all' === $batch_filter ) {
			return 'All';
		}

		if ( ! preg_match( '/^(\d{4})-(\d{2})$/', (string) $batch_filter, $matches ) ) {
			return 'All';
		}

		$timestamp = strtotime( $matches[1] . '-' . $matches[2] . '-01 00:00:00' );

		return $timestamp ? date_i18n( 'F Y', $timestamp ) : 'All';
	}

	private function get_batch_filter_date_range( $batch_filter ) {
		if ( 'all' === $batch_filter ) {
			return array();
		}
		...
		$start_timestamp = strtotime( sprintf( '%04d-%02d-01 00:00:00', $year, $month ) );
		if ( ! $start_timestamp ) {
			return array();
		}
